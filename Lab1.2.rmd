...
**Respuesta:** Los atributos cualitativos se refieren a cualidades de lo que se esté midiendo, tomando de ejemplo a una persona, algunos atributos cualtitativos podrían ser color de piel, estado civil, comida favorita. Los cuantitativos son los atributos medibles numéricamente, por ejemplo la altura, peso, saldo en la cuenta, etc. Se pueden transformar el uno al otro siempre y cuando la codificación que se use tenga sentido para lo que se vaya a usar, por ejemplo podemos convertir el color de piel a un valor RGB cuantitativo, pero no sería de mucha utilidad, en cambio podríamos usar el saldo en la cuenta para definir estratos socioeconómicos cualitativos, que sí podría servir para analizar.
...
**Respuesta:** Una matriz de correlación muestra todos los coeficientes de correlación entre las columnas de un dataframe, así permite visualizar rápidamente cuales atributos tienen mayor o menor correlación.

*5. ¿Cuál es la utilidad de las visualizaciones en el análisis de datos? Ejemplifique.*

**Respuesta:** Las visualizaciones ayudan a representar los datos dándole énfasis al fenómeno que se quiera estudiar, por lo que permite entregar la información mucho más rápido y claramente, si es que la visualización está bien hecha. Por ejemplo si quiero saber cuál es el país con más PIB y como se compara con el resto, podría recorrer una tabla comparando cada país para descubrir el que tiene más, o podría poner a los países en un gráfico, y así sabría inmediatamente el país con más PIB, y como se compara con el resto.
...
```{r}
round(cor(numericos[,1:4]),3)
```

user_followers es el atributo mas correlacionado, ya que su coeficiente de correlación es el más alto, lo que quiere decir que mientras mas seguidores, más probabilidad de que el usuario esté verificado.
